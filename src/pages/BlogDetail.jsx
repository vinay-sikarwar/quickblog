...
const BlogDetail = () => {
  ...
  useEffect(() => {
    if (id) {
      const unsubscribe = commentService.subscribeToCommentsRealtime(
        id,
        (commentsData) => {
  ...
  return (
    <div className="relative overflow-x-hidden">
      ...
      <div className="text-center mt-20 text-gray-600 px-4">
        ...
        <h1 className="text-2xl md:text-4xl lg:text-5xl font-semibold max-w-2xl mx-auto text-gray-800 text-balance text-center">
          {blog.title}
        </h1>
        <h2 className="my-4 text-base sm:text-lg max-w-lg mx-auto text-gray-600 text-center px-2">
          {blog.subTitle}
        </h2>
      ...
      <div className="px-4 sm:px-6 md:px-10 max-w-5xl mx-auto my-10 mt-6">
        <img
          src={blog.imageURL}
          alt={blog.title}
          className="rounded-3xl mb-5 w-full object-cover max-h-[450px] sm:max-h-[550px]"
        />
        <div
          ...
          {commentsLoading ? (
            ...
            <div className="flex flex-col gap-4">
              {comments.map((comment) => (
                <div
                  key={comment.id}
                  className="relative bg-primary/2 border border-primary/5 w-full rounded p-4 text-gray-600"
                >
                  <div className="flex items-center gap-2 mb-2">
                    <img src={assets.user_icon} alt="" className="w-6" />
                    <p className="font-medium break-all">{comment.userEmail}</p>
                  </div>
        ...
        {/* Share Section */}
        <div className="my-24 max-w-3xl mx-auto px-2">
